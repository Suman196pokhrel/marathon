...
- **Of references**: the list holds pointers, not objects. That is why a list can mix types, and why `[[0]*3]*3` breaks (the three rows are the same list, a mutability problem).

### Why append is O(1) but insert(0,x) is O(n)
`append(x)` drops it into the next free slot. One write, done. O(1).
...
`extend` takes any iterable, so `a.extend("ab")` adds `'a'` and `'b'`. Surprises people.
...
```python
people.sort(key=lambda p: (p.dept, p.name))     # dept, then name within each dept
```
